...
Current Position: Module 4, Stage 5 — STARTED 2026-08-27 (Fargate path). Stage 4 CLOSED
  ...
  The creation CTE + `cardinality > 0` predicate is now PROVEN, not just built. Validator
  restored. Added an explicit throw when the CTE returns no job id, so the stack trace
  names the invariant instead of looking like a CTE bug (provable — it just fired).
  (2) Shadowed `error` in both nested catches renamed (`stampError`, `writeError`).
  Traced first: JS block-scopes catch params, so `throw error` after the nested catch
  already referred to the ORIGINAL chunk error — hygiene, not a fix.
  (3) `console.log("premanentErrors")` deleted (duplicate of the `chunk_failed` JSON line).
  (4) NOTES-VS-CODE DRIFT found and resolved IN FAVOUR OF THE CODE: 08-24 said permanent =
  "guard the write, continue". The code only `continue`s when the stamp write SUCCEEDS; a
  failed stamp falls through to `throw error` and the job retries. That is correct: a
  failed stamp means the plumbing is down, the 20 items are still `pending`, and the retry
  re-hits the 23xxx and stamps them. Swallowing would `continue` past 20 pending items no
  attempt would revisit, and the job would end `completed`/`partial` with them abandoned.
  Amended the 08-24 D-log row and F-13.
...
Verified this session (2026-08-18) — cold-start code read:
  ...
  - INNER CATCH mislabels transient failures (new, carried): when a chunk
    ...
    `err.code`: `23xxx` permanent (stamp; `continue` only if the stamp lands, else
    rethrow — see 08-27), everything else
...
- ~~wrap job creation in a transaction (decided 08-18, not yet built)~~ — **BUILT
  ...
  **PROVEN 2026-08-27** — validator relaxed, POST `[]`, `bulk_jobs` count unchanged.
...
- ~~Cleanup: stray `console.log("premanentErrors", ...)` left in the chunk catch (typo and
  ...
  the nested guard's catch — rename to `writeError`.~~ — **DONE 2026-08-27** (both nested
  catches renamed: `stampError`, `writeError`).
...
- **Fix:** classify on `err.code` at the catch.
  - `23xxx` (integrity constraint) → PERMANENT: stamp the 20 `failed` (keeping the
    ...
    try/catch; on success `continue` the loop; on failure RETHROW — the items are
    still `pending`, the retry re-hits the same `23xxx` and stamps them (corrected
    2026-08-27; the notes had said swallow). Job carries on and reaches a real verdict.
